...
def boxplot1(data_boxplot, x_label, y_label, tick):
    ...
    if type(data_boxplot[0]) != list:
        x_scatter = np.full_like(data_boxplot, 1) + np.random.uniform(-0.075, 0.075, len(data_boxplot))
        ax.scatter(x_scatter, data_boxplot, color='red', alpha=1, zorder=3, s= 15)
    else:
        for i, box_data in enumerate(data_boxplot, start=1):
            x_scatter = np.full_like(box_data, i) + np.random.uniform(-0.1, 0.1, len(box_data))
...
for k, ax in zip(values_of_k, axs):
    boxplot4(data_boxplot, None, 'Nº de bolinhas', k = k, ax = ax)
    ax.set_xlabel(f'k = {k}')
